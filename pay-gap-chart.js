...
d3.csv("https://dev.statistik.zh.ch:3000/STAT/LSE_Berufsgruppen_Vis/raw/branch/master/LSE2018_Berufsgruppe.csv").then((csv) => {
  ...
  sortContainer
    ...
    .on("change", function () {
      ...
      sortData();
      render(false); // Changer sort order can benefit from transition to see how occupations' order changes
    })
    .selectAll("option")
    .data(sortOptions)
    .join("option")
  ...
  // Main g group containers rows of occupations
  const g = svg
    .append("g")
  ...
  function render(withoutTransition) {
    ...
    // Occupation column
    g.selectAll(".occupation-name")
      .data(sortedData, (d) => d.occupation)
      .join("text")
      .attr("class", "occupation-name")
      .attr("y", rowHeight / 2)
      .text((d) => d.occupation)
      .call(wrap)
      .transition()
